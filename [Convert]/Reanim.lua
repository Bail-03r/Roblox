...
return end
if game.Players.LocalPlayer.Character:FindFirstChild("Reanim") or workspace:FindFirstChild("Reanim") then
    game.StarterGui:SetCore("ChatMakeSystemMessage", {
        Text = "You are already reanimated!.",
        Color = Color3.fromRGB(255, 10, 70),
        TextSize = 18,
        Font = Enum.Font.Ubuntu
    });
...
game:WaitForChild("Run Service").Heartbeat:Connect(function()
    if Reanimated == true then
        if Character.Reanim then
            Character.Torso.CFrame=Character.Reanim.Torso.CFrame
        end
...
CopyCharacter.Name = "Reanim"

for k,l in pairs(Character.Reanim:GetDescendants()) do 
...
game:GetService("UserInputService").JumpRequest:connect(function(t)
    if CopyCharacter.Humanoid.FloorMaterial~=Enum.Material.Air then 
        ...
        game.Players.LocalPlayer.Character.Reanim:FindFirstChildOfClass('Humanoid').Sit=false 
    end 
end)
